...
const PHASE_LABELS = ['Explicado', 'Entrenado', 'Entendido', 'Dominado'] as const
const PHASE_KEYS = [
  'phaseExplained',
  'phaseTrained',
...
const PHASE_INFO: Array<{
...
}> = [
  {
    key: 'phaseExplained',
    label: 'Explicado',
    description: 'Tu coach te lo ha explicado en clase. Sabes que es y para que sirve.',
  },
  {
    key: 'phaseTrained',
    label: 'Entrenado',
    description: 'Lo has practicado en un ejercicio dirigido. Todavía piensas antes de usarlo.',
  },
  {
    key: 'phaseUnderstoodExplained',
    label: 'Entendido',
    description: 'Te sale bien cuando te concentras, pero necesitas un segundo para construirlo.',
  },
  {
    key: 'phaseUsed',
    label: 'Dominado',
    description: 'Te sale sola. Tu coach te lanza una pregunta en clase y sale bien.',
  },
]
...
export function CoachingV3SessionBoard({
  ...
  const [reportDraftImageFile, setReportDraftImageFile] = useState<File | null>(null)
  ...
  const [classDrafts, setClassDrafts] = useState<Record<string, {
    classRole: string
  ...
  useEffect(() => {
    ...
    for (const classRow of board.classes) {
      ...
      nextDrafts[key] = {
        ...
        coachGuideline1: classRow.coachGuideline1 || '',
        coachGuideline2: classRow.coachGuideline2 || '',
        coachGuideline3: classRow.coachGuideline3 || '',
  ...
  const allCompletedUntilSelected = (board?.focuses || []).filter(
    (focus) => !focus.archivedAt && focus.periodNumber <= selectedPeriod && isCompleted(focus),
  )
  ...
  const aggregatedTasks: AggregatedTask[] = selectedClasses.flatMap((classRow) => {
    const rows = [
      {
        title: classRow.coachGuideline1 || `Tarea ${classRow.classIndex}.1`,
        prompt: classRow.studentGuidelineResponse1 || '',
      },
      {
        title: classRow.coachGuideline2 || `Tarea ${classRow.classIndex}.2`,
        prompt: classRow.studentGuidelineResponse2 || '',
      },
      {
        title: classRow.coachGuideline3 || `Tarea ${classRow.classIndex}.3`,
  ...
  const attemptByFocusId = new Map(selectedAttempts.map((row) => [row.focusId, row]))
  ...
  const titleRecorrido = mode === 'coach' ? 'Recorrido del alumno' : 'Tu recorrido'
  ...
  const isScheduledLiveNow = hasClassLink && selectedClasses.some((classRow) => {
    ...
    try {
      ...
      if (reportDraftImageFile) {
        ...
        if (board.periodReport?.reportImagePath) {
          await deleteCoachingClassReportImage(board.periodReport.reportImagePath)
        }
        nextImagePath = null
      }
  ...
  const handleCreateFocus = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'No se pudo crear el foco.')
    } finally {
  ...
  const handleSaveClass = async (
    ...
    try {
      const updatedClass = await upsertCoachingV2ClassCoachGuidelines({
        ...
        scheduledAt: toIsoFromDateAndTime(draft.scheduledDate, draft.scheduledTime) || null,
          ...
          prev
            ? {
                ...
                classes: prev.classes
                  ...
                  .sort(
                    (a, b) =>
                      a.periodNumber - b.periodNumber || a.classIndex - b.classIndex,
                  ),
              }
            : prev,
        )
      ...
      toast.error(err instanceof Error ? err.message : 'No se pudo guardar la clase.')
    } finally {
  ...
  const handleToggleFocus = async (
    focus: CoachingV2Focus,
    phase: 'phaseExplained' | 'phaseTrained' | 'phaseUnderstoodExplained' | 'phaseUsed',
  ) => {
    if (isSelectedPeriodClosed) {
      toast.error('La semana está cerrada. Ya no se puede editar.')
      return
    ...
    try {
      ...
      setBoard((prev) =>
        prev
          ? {
              ...prev,
              focuses: prev.focuses.map((row) => (row.id === updated.id ? updated : row)),
            }
          : prev,
      )
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'No se pudo actualizar foco.')
    }
  }
  ...
  const handleRegenerateTraining = async (focus: CoachingV2Focus) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : 'No se pudo regenerar el entrenamiento.',
      )
    } finally {
  ...
  if (loading) {
    return <p className='text-sm text-muted-foreground'>Cargando coaching...</p>
  }

  if (!board) {
    return <p className='text-sm text-muted-foreground'>Sin datos de coaching.</p>
  }
  ...
  return (
    <section className='mx-auto w-full max-w-6xl [--v3-bg:oklch(0.98_0.01_220)] [--v3-card:oklch(1_0_0)] [--v3-line:oklch(0.9_0.02_220)] [--v3-text:oklch(0.24_0.03_230)] [--v3-muted:oklch(0.51_0.03_230)] [--v3-cyan:oklch(0.76_0.13_210)] [--v3-gold:oklch(0.79_0.11_78)] dark:[--v3-bg:oklch(0.2_0.04_230)] dark:[--v3-card:oklch(0.25_0.04_230)] dark:[--v3-line:oklch(0.36_0.04_230)] dark:[--v3-text:oklch(0.94_0.01_230)] dark:[--v3-muted:oklch(0.73_0.03_230)]'>
      <div className='rounded-3xl border p-6 shadow-sm md:p-8' style={{ background: 'radial-gradient(130% 90% at 80% -15%, color-mix(in oklab, var(--v3-cyan) 8%, transparent 92%), transparent), var(--v3-bg)', borderColor: 'var(--v3-line)', color: 'var(--v3-text)' }}>
        <header className='flex flex-wrap items-end justify-between gap-5 border-b pb-5' style={{ borderColor: 'var(--v3-line)' }}>
          <div>
            <p className='text-xs' style={{ color: 'var(--v3-muted)' }}>Coaching ICA</p>
          ...
          <div className='text-sm' style={{ color: 'var(--v3-muted)' }}>
            <div className='flex items-center gap-2'>
              <span className='inline-flex size-6 items-center justify-center rounded-full border border-cyan-400/30 bg-cyan-400/20 text-xs font-semibold text-cyan-300'>L</span>
              <span className='inline-flex size-6 items-center justify-center rounded-full border border-amber-500/35 bg-amber-500/15 text-xs font-semibold text-amber-300'>
                {(board.coachers?.selectedCoachDisplayName || coachDisplayName || 'Eve').slice(0, 1).toUpperCase()}
              </span>
              <p>
                Luis y {board.coachers?.selectedCoachDisplayName || coachDisplayName || 'Eve'}
              </p>
            </div>
            <div className='mt-2'>
              {hasClassLink ? (
                ...
                >
                  <span
                    ...
                    style={{
                      background:
                        classLinkState === 'live' ? '#ef4444' : 'var(--v3-cyan)',
                    }}
                  />
                  {classLinkState === 'live' ? 'Clase en vivo ahora' : 'Link a tu clase'}
                </a>
              ) : (
                <span
                  className='inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-medium opacity-80'
                  style={{
                    borderColor: 'color-mix(in oklab, var(--v3-muted) 30%, var(--v3-line) 70%)',
                    background: 'color-mix(in oklab, var(--v3-line) 55%, transparent 45%)',
                    color: 'var(--v3-muted)',
                  }}
                >
                  <span
                    className='inline-block size-2 rounded-full'
                    style={{
                      background: 'color-mix(in oklab, var(--v3-muted) 70%, transparent 30%)',
                    }}
                  />
        ...
        <section className='mt-7'>
              ...
              return (
                ...
                >
                  <span
                   ...
                   />
                   <span className='mt-1 block text-[11px]' style={{ color: !isActivated ? 'color-mix(in oklab, var(--v3-muted) 55%, transparent 45%)' : isSelected ? 'var(--v3-text)' : 'var(--v3-muted)' }}>
                     {period}
                   </span>
                 </button>
              )
          ...
          {mode === 'coach' && (
            <div className='mt-3 flex flex-wrap gap-2'>
              ...
              >
                {savingPeriodAction ? 'Activando...' : 'Activar siguiente semana'}
              </Button>
              <Button
                type='button'
                variant='destructive'
        ...
        <section className='mt-10'>
          <div className='mb-4 flex items-baseline justify-between gap-3'>
            <h2 className='text-2xl font-semibold tracking-tight'>Los tres focos</h2>
            ...
            {focusSlots.map((focus, idx) => {
              if (!focus) {
                return (
                  <Card
                    ...
                    style={{ borderColor: 'var(--v3-line)', background: 'color-mix(in oklab, var(--v3-card) 88%, black 12%)' }}
                    onClick={() => {
                      if (mode === 'coach' && canAddFocus && canEditSelectedPeriod) setFocusModalOpen(true)
                    }}
                  >
                    <p className='text-sm font-medium' style={{ color: 'var(--v3-muted)' }}>Hueco libre</p>
              ...
              const selectedPhaseIdx =
                typeof selectedFocusPhase[focus.id] === 'number'
                  ...
                  : suggestedPhaseIdx
              const selectedPhaseInfo = PHASE_INFO[selectedPhaseIdx] || PHASE_INFO[suggestedPhaseIdx]
              const selectedPhaseDone = Boolean(focus[selectedPhaseInfo.key])
              const canToggleSelectedPhase = canTogglePhase(focus, selectedPhaseInfo.key)
              const focusExercise = exerciseByFocusId.get(focus.id)
              const focusAttempt = attemptByFocusId.get(focus.id)
              const trainedPhaseSelected = selectedPhaseInfo.key === 'phaseTrained'
              const canUseTrainedActions = trainedPhaseSelected && focus.phaseExplained
              ...
              return (
                <Card key={focus.id} className='rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md' style={{ borderColor: 'var(--v3-line)', background: 'var(--v3-card)' }}>
                  <div className='flex items-center justify-between gap-2'>
                    <span className='text-xs' style={{ color: 'var(--v3-muted)' }}>
                      Abierto en semana {focus.periodNumber}
                    </span>
                    <div className='flex items-center gap-2'>
                      <span className='text-sm font-semibold'>{progress}/4</span>
                      <Button
                        size='icon'
                        variant='outline'
                  ...
                  <h3 className='mt-2 text-lg font-semibold leading-tight'>{focus.focusTitle}</h3>
                  ...
                  <p className='mt-3 min-h-14 text-xs leading-relaxed' style={{ color: 'var(--v3-muted)' }}>
                    <b style={{ color: 'var(--v3-text)' }}>{selectedPhaseInfo.label}.</b>{' '}
                      ...
                      <Button
                        ...
                        style={{
                          borderColor: selectedPhaseDone
                            ...
                            : 'var(--v3-line)',
                          color: selectedPhaseDone ? 'var(--v3-cyan)' : 'var(--v3-muted)',
                          background: selectedPhaseDone
                            ? 'color-mix(in oklab, var(--v3-cyan) 12%, transparent 88%)'
                            : 'transparent',
                        }}
                        disabled={!canToggleSelectedPhase || !canEditSelectedPeriod}
                        onClick={() => void handleToggleFocus(focus, selectedPhaseInfo.key)}
                      >
                        {selectedPhaseDone ? <CheckIcon className='size-3.5' /> : null}
                    ...
                    <>
                      {trainMissing || trainError ? (
                        <Button
                          ...
                          disabled={regeneratingFocusId === focus.id || !canEditSelectedPeriod}
                        ...
                        <Button
                          ...
                          className='mt-4 w-full'
                          disabled={!trainReady || !canEditSelectedPeriod}
                          onClick={() => {
                            if (!trainReady || !canEditSelectedPeriod) return
                            navigate(
                              getCoachingV2ExerciseRoute(sessionId, selectedPeriod, focus.id),
                            )
                          }}
                        >
                          {trainReady ? trainButtonLabel : 'Generando entrenamiento...'}
                        </Button>
                      )}
                      {!trainReady && !trainError && !trainMissing && (
                        <p className='mt-2 text-center text-xs' style={{ color: 'var(--v3-muted)' }}>
                          {trainPreparing
          ...
          {allCompletedUntilSelected.length > 0 && (
            <div className='mt-4 flex flex-wrap items-center gap-2'>
              <span className='text-xs' style={{ color: 'var(--v3-muted)' }}>Ya dominados:</span>
              {allCompletedUntilSelected.map((focus) => (
                <Badge key={`done-${focus.id}`} variant='outline' className='rounded-full border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300'>
        ...
        <section className='mt-10'>
          <div className='mb-4 flex items-baseline justify-between gap-3'>
            <h2 className='text-2xl font-semibold tracking-tight'>Tareas semanales</h2>
            <p className='text-xs' style={{ color: 'var(--v3-muted)' }}>
              Seis tareas entre las dos clases. Al completarlas se abre tu reporte.
            </p>
          </div>

            <Card className='rounded-3xl border p-5' style={{ borderColor: 'var(--v3-line)', background: 'linear-gradient(180deg, color-mix(in oklab, var(--v3-cyan) 12%, var(--v3-card) 88%), var(--v3-card))' }}>
            <div className='mb-4 flex items-center justify-between'>
              ...
              <p className='text-sm'>
                <b className='text-4xl font-bold leading-none'>{completedTasks}</b>{' '}
                de 6 tareas
              </p>
            </div>
            ...
            <div className='relative mt-2'>
              ...
              <div className='relative grid grid-cols-7 items-start gap-2'>
                {Array.from({ length: 6 }, (_, idx) => {
                  const task = aggregatedTasks.find((item) => item.index === idx + 1)
                  const done = Boolean(task?.done)
                  return (
                    <div key={`spine-${idx + 1}`} className='flex flex-col items-center gap-2 text-xs'>
                      <span
                      ...
                      <span style={{ color: 'var(--v3-muted)' }}>Clase {idx < 3 ? 1 : 2}</span>
                    </div>
                  )
                })}
            ...
            <div className='mt-5 border-t pt-4' style={{ borderColor: 'var(--v3-line)' }}>
              <p className='text-sm' style={{ color: 'var(--v3-muted)' }}>
                ...
                {nextPendingTask ? `, de la clase ${nextPendingTask.classIndex}.` : '.'}
              </p>
            </div>
          </Card>
        </section>
        ...
        <section className='mt-10'>
          <div className='mb-4 flex items-baseline justify-between gap-3'>
            <h2 className='text-2xl font-semibold tracking-tight'>{titleClases}</h2>
            <p className='text-xs' style={{ color: 'var(--v3-muted)' }}>
              {classesSubtitle}
            </p>
          </div>
          ...
          <div className='grid gap-3 md:grid-cols-2'>
            {[1, 2].map((slot) => {
              const classRow = selectedClasses.find((row) => row.classIndex === slot) || null
              const classRoleLabel =
                classRow?.title && classRow.title.trim() !== `Clase ${slot}`
                  ? classRow.title.trim()
                  : ''

              const classCoachName =
                classRow?.assignedByCoachUserId
                  ? board.coachers?.classAssignedCoachDisplayNameByClassIndex?.[String(slot)] || null
                  : null

              const classTasks = aggregatedTasks.filter((task) => task.classIndex === slot)
              const key = `${selectedPeriod}-${slot}`
              const fallbackScheduled = toDateAndTimeFromIso(classRow?.scheduledAt || null)
              ...
              return (
                <Card key={`class-${slot}`} className='rounded-2xl border p-4' style={{ borderColor: 'var(--v3-line)', background: 'var(--v3-card)' }}>
                  <div className='flex items-start justify-between gap-3'>
                    ...
                    {mode === 'student' && classCoachName ? (
                      <Badge variant='outline' className='rounded-full border-amber-500/45 bg-amber-500/10 text-amber-700 dark:text-amber-300'>
                        {classCoachName}
                      </Badge>
                    ) : null}
                  </div>
                  ...
                  {embedUrl ? (
                    <div className='mt-3 overflow-hidden rounded-xl border' style={{ borderColor: 'var(--v3-line)' }}>
                      <iframe
                        src={embedUrl}
                    ...
                    >
                      ...
                      {classRow?.loomUrl ? 'Abrir grabacion en Loom' : 'Grabacion pendiente'}
                    </div>
                  )}
                  ...
                  {mode === 'student' ? (
                    <Accordion type='single' collapsible className='mt-4'>
                      {classTasks.map((task) => {
                        ...
                        const responseKey = localIndex === 1 ? 'response1' : localIndex === 2 ? 'response2' : 'response3'
                        ...
                        return (
                          <AccordionItem key={`task-acc-${slot}-${task.index}`} value={`task-${slot}-${task.index}`}>
                            <AccordionTrigger className='hover:no-underline disabled:cursor-not-allowed disabled:opacity-60' disabled={!isAssigned}>
                              <div className='flex w-full items-start gap-3 text-left'>
                                ...
                                >
                                  {isDone ? <CheckIcon className='size-3.5' /> : task.index}
                                </span>
                                <div>
                                  <p className='text-base font-semibold leading-tight'>
                                    {isAssigned ? task.title : 'Pendiente de asignar'}
                                  </p>
                                  <p className='text-xs text-muted-foreground'>
                                    {isAssigned ? (isDone ? 'Hecha' : 'Por completar') : 'Pendiente de asignar por tu coach'}
                                  </p>
                                </div>
                              </div>
                            </AccordionTrigger>
                                ...
                                <>
                                  <Textarea
                                    ...
                                    onChange={(event) =>
                                      setClassDrafts((prev) => ({
                                        ...prev,
                                        [key]: { ...draft, [responseKey]: event.target.value },
                                      }))
                                    }
                                    rows={3}
                                  ...
                                  <div className='mt-2 flex items-center gap-2'>
                                    <Button
                                      ...
                                      disabled={
                                        ...
                                        savingClassKey === `${key}-r${localIndex}` ||
                                        !responseValue.trim()
                                      }
                                    >
                                      {savingClassKey === `${key}-r${localIndex}` ? 'Guardando...' : 'Guardar'}
                                    </Button>
                                    {task.done ? (
                                      <span className='text-xs text-cyan-300'>Hecha</span>
                                    ) : null}
                                  </div>
                                </>
                              ) : null}
                  ...
                  {mode === 'coach' && (
                    <div className='mt-4 space-y-2 rounded-xl border p-3' style={{ borderColor: 'var(--v3-line)' }}>
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setClassDrafts((prev) => ({
                            ...prev,
                            [key]: { ...draft, assignedCoachUserId: event.target.value },
                          }))
                        }
                            ...
                            [
                              {
                                ...
                                name: board.coachers?.primaryCoachDisplayName || 'Luis',
                              },
                              {
                      ...
                      <div className='grid gap-2 sm:grid-cols-2'>
                        <Input
                          ...
                          onChange={(event) =>
                            setClassDrafts((prev) => ({
                              ...prev,
                              [key]: { ...draft, scheduledDate: event.target.value },
                            }))
                          }
                          disabled={!canEditSelectedPeriod}
                        />
                        <Input
                          ...
                          onChange={(event) =>
                            setClassDrafts((prev) => ({
                              ...prev,
                              [key]: { ...draft, scheduledTime: event.target.value },
                            }))
                          }
                          disabled={!canEditSelectedPeriod}
                        />
                      ...
                      <Accordion type='single' collapsible>
                        {([
                          ...
                          const isAnswered = Boolean(item.responseValue?.trim())
                          return (
                            <AccordionItem
                            ...
                            >
                              <AccordionTrigger className='hover:no-underline'>
                                <div className='flex w-full items-center gap-2 text-left'>
                                  ...
                                  >
                                    {isAnswered ? <CheckIcon className='size-3' /> : item.absoluteIndex}
                                  </span>
                                  <span className='text-sm font-medium'>Tarea {item.absoluteIndex}</span>
                                </div>
                              </AccordionTrigger>
                              <AccordionContent className='space-y-2'>
                                <Textarea
                                  ...
                                  onChange={(event) =>
                                    setClassDrafts((prev) => ({
                                      ...prev,
                                      [key]: { ...draft, [item.guidelineKey]: event.target.value },
                                    }))
                                  }
                                  rows={2}
                                ...
                                />
                                {isAnswered ? (
                                  <div className='rounded-md border p-2 text-xs' style={{ borderColor: 'var(--v3-line)' }}>
                                    <p className='font-medium text-foreground'>Respuesta del alumno</p>
                                    <p className='mt-1 text-muted-foreground'>{item.responseValue}</p>
                                  </div>
                                ) : (
                                  <p className='text-xs text-muted-foreground'>Sin respuesta del alumno.</p>
                                )}
                              </AccordionContent>
                            </AccordionItem>
                          )
                      ...
                      <Button
                        ...
                        disabled={savingClassKey === key || !canEditSelectedPeriod}
                      >
                        {savingClassKey === key ? 'Guardando...' : 'Guardar clase'}
                      </Button>
                    </div>
                  )}
                </Card>
          ...
          >
            {reportStatus === 'blocked' ? (
              <div className='flex items-center gap-4'>
                <span
                  ...
                  style={{
                    borderColor: 'color-mix(in oklab, var(--v3-line) 70%, var(--v3-cyan) 30%)',
                    background: 'color-mix(in oklab, var(--v3-card) 86%, black 14%)',
                    color: 'var(--v3-muted)',
                  }}
                >
              ...
              <>
                <div className='flex items-center justify-between gap-3'>
                  <h3 className='text-xl font-semibold'>Reporte de la semana {selectedPeriod}</h3>
                  <Badge variant='outline' className='rounded-full'>
                    {reportStatus === 'available' ? 'Disponible' : 'En preparación'}
                  </Badge>
                </div>
            ...
            {mode === 'coach' && (
              <div className='mt-4 space-y-3 rounded-xl border p-3' style={{ borderColor: 'var(--v3-line)' }}>
                <p className='text-sm font-medium'>Editar reporte de la semana</p>
                <Textarea
                ...
                <div className='space-y-2'>
                  ...
                  {removeReportImage && (
                    <p className='text-xs text-muted-foreground'>La imagen se eliminará al guardar.</p>
                  )}
                </div>
                ...
                <div className='flex flex-wrap gap-2'>
                  <Button
                    ...
                    disabled={savingReport || !userId || !canEditSelectedPeriod}
                  >
      ...
      <Dialog open={Boolean(openFocusComment)} onOpenChange={(open) => !open && setOpenFocusComment(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Comentario del foco</DialogTitle>
          </DialogHeader>
          <p className='text-sm text-muted-foreground'>
            {selectedFocuses.find((focus) => focus.id === openFocusComment)?.focusComment ||
              'Sin comentario para este foco.'}
          </p>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className='space-y-2'>
            ...
            <Button
              ...
              disabled={savingFocus || !focusDraftTitle.trim() || !canEditSelectedPeriod}
            >
        ...
        <DialogContent className='max-h-[85vh] overflow-y-auto sm:max-w-3xl'>
              ...
              {(['Reconocer', 'Construir', 'En conversacion'] as const).map((name, idx) => {
                ...
                return (
                  <TabsContent key={`review-tab-${value}`} value={value} className='mt-3 space-y-2'>
                    {failures.length === 0 ? (
                      <p className='text-sm text-muted-foreground'>Sin fallos registrados en esta sección.</p>
                    ) : (
                        ...
                        return (
                          <div key={`failure-${value}-${itemIdx}`} className='rounded-md border p-3 text-sm'>
                            <p className='font-medium'>
                              {typeof row.question === 'string' ? row.question : 'Pregunta'}
                            </p>
                            <p className='mt-1 text-xs text-muted-foreground'>
                              Respondio: {typeof row.mine === 'string' ? row.mine : '—'}
                            </p>
                            <p className='text-xs text-emerald-700'>
                              Esperado: {typeof row.expected === 'string' ? row.expected : '—'}
                            </p>
                          </div>
                        )
                      })
          ...
          {openReviewAttempt && (
            <div className='space-y-1 rounded-md border bg-muted/20 p-3 text-xs'>
              <p>Total: {openReviewAttempt.scoreCorrect}/{openReviewAttempt.scoreTotal}</p>
