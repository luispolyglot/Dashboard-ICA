...
export const TRACKERS_MIN_YEAR = 2024
export const TRACKERS_MIN_MONTH = 6
...
export function getTrackerInsertErrorMessage(error: unknown): string {
  ...
  if (error.message.includes('TRACKER_MONTH_TOO_OLD')) {
    return 'Solo se admiten trackers desde junio de 2024.'
  }
  ...
  if (error.message.includes('TRACKER_MONTH_MUST_BE_MONTH_START')) {
    return 'El mes seleccionado no es valido.'
  }
...
export async function createImprovementTracker(
  ...
  const userId = await getCurrentUserId()
  if (!userId) {
    throw new Error('Necesitas iniciar sesion para guardar trackers.')
  }
