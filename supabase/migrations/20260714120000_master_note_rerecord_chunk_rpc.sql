...
begin
  ...
  select mn.user_id, mn.state
  into v_note_user_id, v_note_state
  ...
  if v_note_state <> 'open' then
    raise exception 'MASTER_NOTE_NOT_OPEN';
  end if;

  select c.phrase_generation_id, c.storage_path
  into v_chunk_phrase_generation_id, v_chunk_storage_path
...
grant execute on function public.rerecord_master_note_chunk(
  uuid,
  uuid,
  uuid,
  uuid,
