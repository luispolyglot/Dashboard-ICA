...
begin
  ...
  -- Progress shown to the user: activations accumulated in the current Friday-to-Friday window.
  ...
  -- Unlock rule: to unlock this week, require 20 activations in the previous window.
  select count(distinct pla.lexicard_id)::integer
  into previous_unlock_count
  from public.phrase_lexicard_activations pla
  where pla.user_id = current_user_id
  ...
  previous_unlock_count := coalesce(previous_unlock_count, 0);
  progress_unlock := previous_unlock_count >= 20;
